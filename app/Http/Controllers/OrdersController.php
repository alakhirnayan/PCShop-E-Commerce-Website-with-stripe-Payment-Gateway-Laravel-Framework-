<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Order;
use Auth;
class OrdersController extends Controller
{
    public function allOrders()
    {
    	$allOrders = Order::all();
    	return view('admin.order.allOrders',compact('allOrders'));
    }
    public function statusOrder(Request $request, $id)
    {
    	$status = Order::find($id);
    	$status->status = $request->status;
    	$status->save();

    	return back();
    }

    public function pendingOrders()
    {
    	$pendingOrders = Order::where('status','0')->get();
    	return view('admin.order.pendingOrders',compact('pendingOrders'));
    }

    public function deliveredOrders()
    {
    	$deliveredOrders = Order::where('status','1')->get();
    	return view('admin.order.deliveredOrders',compact('deliveredOrders'));
    }
    public function myApi()
    {
        $data = Order::all();
        return response()->json($data);
    }

    public function uallOrders(Request $request)
    {
        $id = Auth::user()->id;
        $allOrders = Order::where('user_id',$id)->get();
        return view('user.order.allOrders',compact('allOrders'));
    }
    public function udeliveredOrders()
    {
        $id = Auth::user()->id;
        $deliveredOrders = Order::where('status','1')->where('user_id',$id)->get();
        return view('user.order.deliveredOrders',compact('deliveredOrders'));
    }

    
}
